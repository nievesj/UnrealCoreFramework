...
public:
	...
	/** Cancel all active tweens on this widget. Leaves widget at last applied state. */
	...
	static void RegisterTweenState(TSharedPtr<FCoreTweenState> State);

private:
	/** Prune finished entries from the tracking array. */
	static void PruneFinished();

	static TArray<TSharedPtr<FCoreTweenState>> ActiveTweenStates;
};
